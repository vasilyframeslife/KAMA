...
// Большое фото на странице квартиры раньше всегда запрашивалось в ширине
// 1600px, даже на телефоне с экраном 380px — это и была основная причина
// медленной загрузки на мобильных. Теперь ширина подбирается под реальный
// экран (с учётом плотности пикселей), но не крупнее разумного предела.
// The big apartment photo used to always request a 1600px-wide file, even on
// a 380px-wide phone screen — that was the main reason photos felt slow to
// load on mobile. Now the width is picked to match the real screen (accounting
// for pixel density), capped at a sane maximum.
function optimalPhotoWidth(){
  const target = Math.round(window.innerWidth * (window.devicePixelRatio || 1));
  return Math.max(700, Math.min(1600, target));
}
// Заранее подгружает все фото галереи в фоне (в объект Image, без вставки в
// DOM), чтобы при свайпе между фото они уже были в кэше браузера и
// открывались мгновенно, а не заново качались с сервера.
// Prefetches every gallery photo in the background (into an Image object, not
// the DOM) so that swiping between photos hits the browser cache instantly
// instead of downloading each one again on demand.
function prefetchGallery(gallery){
  const w = optimalPhotoWidth();
  gallery.forEach(url => { const im = new Image(); im.src = url.replace(/w=\d+/, 'w=' + w); });
}
...
function setGallery(idx){
  ...
  document.getElementById('detailImg').src = a.gallery[galleryIdx].replace(/w=\d+/, 'w=' + optimalPhotoWidth());
...
function openDetail(id){
  ...
  img.style.transform = '';
  img.src = a.gallery[0].replace(/w=\d+/, 'w=' + optimalPhotoWidth());
  prefetchGallery(a.gallery);
...
// На телефоне на каждый кадр скролла и так уходит немало ресурсов браузера
// (сам скролл + перерасчёт layout), поэтому там разбиваем тяжёлые под-обновления
// (padding/шрифт/размер кнопок — они запускают layout reflow) на через кадр,
// а не считаем 60 раз в секунду — именно это раньше "съедало" кадры и
// анимация выглядела рывками именно на мобильных.
// On phones there's already a lot of browser work happening per scroll frame
// (the scroll itself + layout recalculation), so there we spread the expensive
// sub-updates (padding/font-size/button-size — these force a layout reflow)
// across every other frame instead of computing all of them 60 times a second —
// that per-frame reflow cost was exactly what dropped frames on mobile.
const isNarrowViewport = window.matchMedia && window.matchMedia('(max-width: 640px)').matches;
...
// Ссылки на элементы шапки квартиры кэшируются один раз при загрузке страницы
// (а не через document.getElementById на каждый кадр скролла) — эти элементы
// статические и не пересоздаются, так что кэш всегда актуален.
// Header element references are cached once at page load (instead of calling
// document.getElementById on every scroll frame) — these elements are static
// and never recreated, so the cache is always valid.
const heroEls = {
  hero: null, img: null, back: null, fav: null, info: null,
  title: null, addr: null, price: null, label: null
};
function cacheHeroEls(){
  heroEls.hero = document.getElementById('detailHero');
  heroEls.img = document.getElementById('detailImg');
  heroEls.back = document.getElementById('detailBackBtn');
  heroEls.fav = document.getElementById('detailFavBtn');
  heroEls.info = document.getElementById('detailHeroInfo');
  heroEls.title = document.getElementById('detailTitle');
  heroEls.addr = document.getElementById('detailAddr');
  heroEls.price = document.getElementById('detailPrice');
  heroEls.label = document.getElementById('detailBackLabel');
}
...
function updateHeroScroll(){
  ...
  const dv = document.getElementById('detail-view');
  if(!dv.classList.contains('visible')) return;
  const { hero, img, back, fav, info, title, addr, price, label } = heroEls;
  if(!hero || hero.dataset.animating === '1') return;
  ...
  // Эти два свойства — самые дешёвые (height у fixed-элемента не трогает
  // остальной layout страницы, transform — чисто композитинг), их считаем
  // каждый кадр без исключений, это и есть основной визуальный эффект.
  // These two are the cheapest (height on a fixed element doesn't touch the
  // rest of the page's layout; transform is compositor-only) — computed every
  // frame with no exceptions, since this is the core visual effect.
  hero.style.height = (MAX_H - t * range) + 'px';
  hero.classList.toggle('compact', t > 0.96);
  if(img) img.style.transform = `scale(${1 + t * 0.05})`;

  heroFrameCount++;
  const doHeavyUpdate = !isNarrowViewport || (heroFrameCount % 2 === 0);
  if(doHeavyUpdate && back && fav && info && title && price){
...
// ЗАПУСК ПРИЛОЖЕНИЯ / APP BOOTSTRAP
// -----------------------------------------------------------------------
cacheHeroEls();
renderFilters();
renderFaq();
